...
import { fetchFunMovies, type MovieItem } from '../../api/tmdbMovie';
...
const FunMovies: React.FC = () => {
  const [list, setList] = useState<MovieItem[]>([]);

  useEffect(() => {
    fetchFunMovies().then((data) => setList((data.results ?? []).filter((m) => m.poster_path)));
  }, []);

  return (
    <section className="movie-section">
      <h3 className="section-title">코미디 영화</h3>
