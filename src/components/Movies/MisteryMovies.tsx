...
import { fetchMisteryMovies, type MovieItem } from '../../api/tmdbMovie';
...
const MysteryMovies: React.FC = () => {
  const [list, setList] = useState<MovieItem[]>([]);

  useEffect(() => {
    fetchMisteryMovies().then((data) => setList((data.results ?? []).filter((m) => m.poster_path)));
  }, []);

  return (
    <section className="movie-section">
      <h3 className="section-title">스릴러 영화</h3>
