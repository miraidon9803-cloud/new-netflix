...
const BestSeries: React.FC = () => {
  ...
  useEffect(() => {
    console.log('[BestSeries] fetch start');
    ...
    fetchRecentTVSeries2025({
      ...
      .then((data) => {
        console.log('[BestSeries] results array:', data.results);
        console.log('[BestSeries] results count:', data.results?.length ?? 0);
        console.log('[BestSeries] first item:', data.results?.[0]);

        setList(data.results ?? []);
      })
      .catch((err) => console.error('[BestSeries] error:', err));
  }, []);
