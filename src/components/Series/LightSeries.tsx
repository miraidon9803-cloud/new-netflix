...
const LightSeries: React.FC = () => {
  ...
  useEffect(() => {
    console.log('[LightSeries] fetch start');
    setLoading(true);
    setError('');
    ...
    Promise.all([
      ...
      .then(([comedyRes, romanceRes, animationRes]) => {
        const merged = [
          ...(comedyRes.results ?? []),
          ...(romanceRes.results ?? []),
        ...
        console.log('[LightSeries] final count:', randomized.length);

        setList(randomized);
      })
      .catch((err) => {
        console.error('[LightSeries] error:', err);
