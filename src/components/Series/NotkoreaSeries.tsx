...
const ForeignSeries: React.FC = () => {
  ...
  useEffect(() => {
    console.log('[ForeignSeries] fetch start');
    ...
    fetchRecentTVSeries2025({
      ...
      .then((data) => {
        console.log('[ForeignSeries] results array:', data.results);
        console.log('[ForeignSeries] results count:', data.results?.length ?? 0);
        console.log('[ForeignSeries] first item:', data.results?.[0]);

        setList(data.results ?? []);
      })
      .catch((err) => console.error('[ForeignSeries] error:', err));
  }, []);
