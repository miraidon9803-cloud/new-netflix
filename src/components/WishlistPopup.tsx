...
const WishlistPopup: React.FC<WishlistPopupProps> = ({ content, onClose }) => {
  ...
  const { folders, isLoading, loadFolders, addContentToFolder } = useWishlistStore();
  ...
  const handleSave = async () => {
    if (selectedFolderId) {
      await addContentToFolder(selectedFolderId, content);
      onClose();
    }
  };
    ...
    return (
      <div className="wishlist-popup-overlay" onClick={handleOverlayClick}>
        <div className="wishlist-popup-container">
          <p style={{ color: '#fff', textAlign: 'center', padding: '40px 0' }}>로딩 중...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="wishlist-popup-container">
        ...
        {folders.length === 0 ? (
          <>
            ...
            <button className="wishlist-popup-save-btn" onClick={handleGoToWishlist}>
              위시리스트 이동
            </button>
          </>
        ) : (
          <>
            ...
            <div className="wishlist-popup-folder-list">
              {folders.map((folder) => (
                <button
                  key={folder.id}
                  className={`wishlist-popup-folder-item ${selectedFolderId === folder.id ? 'selected' : ''}`}
